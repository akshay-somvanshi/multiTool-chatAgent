...
### Theme first
Always set up a theme before building UI. Define the color scheme with `app.themeColor()`, the type scale with `app.typography(...)`, and design tokens with `app.spacingToken/radiusToken/shadowToken(...)`. Never hardcode hex colors in widgets — use `Colors.primary`, `Colors.secondary`, `Colors.secondaryText`, etc. The full theme/design-system surface (palette, typography, tokens, custom fonts/icons, scrollbar, pull-to-refresh) is in `references/theme_design_system_dsl.dart`. For a smaller "minimum viable theme + form layout" pattern, see `references/styled_profile_dsl.dart`.
